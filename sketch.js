...
function setup() {
  ...
  text_15_0 = new TypingAnimation('삐이~ 삐삐뺴삐삐빼삐!(정말 매우 고맙다는 뜻)');
...
function stage_03() { // 삐빼삐 등장
  ...
  if (moveTime < 50) {
    ...
    imageMode(CENTER);
    image(helperImg,0,0,20+80/50*moveTime,0+80/50*moveTime);
    pop();
  } else {
    imageMode(CENTER);
...
function stage_25() { // 더듬이 나중에
  ...
  if(mouseX<=280 && mouseX>=230 
    && mouseY<360 && mouseY>=230){
      ...
      pop();
      handsOnDeodum = true;
      image(hand[1],mouseX,mouseY,40,40)
    }else {
      handsOnDeodum = false;
...
function cutScene() {
  if (30 < moveTime && moveTime < 60) {
    let x = 355/900*(moveTime-60)**2;
    fill(255);
    imageMode(CORNER);
    image(toons[0],120-x,80,480,320)
    //rect(120-x,80,480,320);
  } else if (moveTime >= 60) {
    fill(255);
    imageMode(CORNER);
    image(toons[0],120,80,480,320)
    //rect(120,80,480,320);
  }

  if (120 < moveTime && moveTime < 150) {
    let x = -355/900*(moveTime-150)**2;
    fill(255);
    imageMode(CORNER);
    image(toons[1],120-x,80,480,320);
    //rect(120-x,80,480,320);
  } else if (moveTime >= 150) {
    fill(255);
    imageMode(CORNER);
    image(toons[1],120,80,480,320);
    //rect(120,80,480,320);
  }

  if (210 < moveTime && moveTime < 240) {
    let x = 355/900*(moveTime-240)**2;
    fill(255);
    imageMode(CORNER);
    image(toons[2],120-x,80,480,320);
    //rect(120-x,80,480,320);
  } else if (moveTime >= 240) {
    fill(255);
    imageMode(CORNER);
    image(toons[2],120,80,480,320);
    //rect(120,80,480,320);
  }

  if (moveTime > 300 && moveTime < 330) {
    let x = -355/900*(moveTime-330)**2;
    fill(255);
    imageMode(CORNER);
    image(toons[3],120-x,80,480,320);
    //rect(120-x,80,480,320);
  } else if (moveTime >= 330) {
    fill(255);
    imageMode(CORNER);
    image(toons[3],120,80,480,320);
    //rect(120,80,480,320);
  }
}
...
function drawDial() {
  ...
  imageMode(CENTER);
  image(arrowImg,260,240,210,115);
  ...
  switch (stage) {
    ...
    case 3: // 삐빼삐 등장
      if (key == ' ') {
        ...
        else if (textStage == 4 && text_03_4.check()) stage = 4;
      }
      break;
    case 4:
      // 활성화된 칸에 한 글자 입력
